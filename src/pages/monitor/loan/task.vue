...
export default {
  ...
  methods: {
    ...
    async init () {
      this.decisionMap = await BS.getOptions('saas_fund_loan_decision')
    },
    getTableData () {
      this.currentTasks = []
      this.params.timeStart = this.cteateDate.start ? Utils.fmtTime(this.cteateDate.start, 'yyyy-MM-dd') : ''
      this.params.timeEnd = this.cteateDate.end ? Utils.fmtTime(this.cteateDate.end, 'yyyy-MM-dd') : ''
      let start = this.cteateDate.start || ''
      let end = this.cteateDate.end || ''
      if (start > end && (![start, end].includes(''))) {
        return this.$refs.toast.show('进件起始日期需小于截止日期')
      }
      ...
      this.loading = true
      FundingApi.loanTaskList(params).then((data) => {
    ...
    getBatch () {
      let params = this.currentTasks.join(',')
       FundingApi.getLoanAssignUsers({ taskIds: params }).then((data) => {
    ...
    ok () {
      ...
      FundingApi.loanAssignTask(params).then((data) => {
    ...
    todoOk (obj) {
      FundingApi.changeLoanTaskStatus({ taskId: obj.taskId }).then(() => {
    ...
    opera (obj) {
      ...
      window.open(`/harbour/static/#/monitor/loan/detail?loanId=${loanId}&proKey=loan`, '_blank')
    },
    single () {
      let taskIds = this.currentTasks.join(',')
      if (this.currentTasks.length < 1) {
      ...
      FundingApi.reAssignLoanTaskBatch({ taskIds }).then((data) => {
