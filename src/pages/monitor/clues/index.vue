    ...
    <section>
      ...
      <s-table :columns="columns" :source="source" :isLoading="loading" >
        <template v-slot:opera="{ row }">
          <div class="opera">
            <a href="javascript:;"  @click="opera(row.item, 'detail')">详情</a>
            <a href="javascript:;"  @click="opera(row.item, 'require')" >需求</a>
          </div>
        </template>
      </s-table>
    </section>
...
export default {
  ...
  methods: {
    ...
    getTableData () {
      ...
      this.loading = true
      FundingApi.cluesList(params).then((data) => {
    ...
    opera (obj, type) {
      ...
      if (type === 'require') {
        window.open(`/harbour/static/#/monitor/apply?requireCode=${requireCode}`, '_blank')
      } else {
        window.open(`/harbour/static/#/monitor/clues/detail?requireCode=${requireCode}&proKey=apply`, '_blank')
      }
    },
    search (status) {
      if (status) {
